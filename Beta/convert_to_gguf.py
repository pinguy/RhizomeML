...
5. Auto-clones, updates, and builds llama.cpp if needed
...
def update_llama_cpp(llama_cpp_dir: Path) -> bool:
    """Pull latest changes from git."""
    logger.info("=" * 60)
    logger.info("Updating llama.cpp from source...")
    logger.info("=" * 60)
    
    try:
        # Check if it's a git repo
        if not (llama_cpp_dir / ".git").exists():
            logger.error(f"Cannot update: {llama_cpp_dir} is not a git repository.")
            return False
        ...
        logger.info("✅ llama.cpp updated successfully!")
        return True
    except subprocess.CalledProcessError as e:
        logger.error(f"Failed to update llama.cpp: {e}")
        return False
    except Exception as e:
        logger.error(f"Error updating llama.cpp: {e}")
        return False
...
def ensure_llama_cpp_built(llama_cpp_dir: Path, cuda_enabled: bool = True, force_update: bool = False) -> bool:
    ...
    if not (llama_cpp_dir / "convert_hf_to_gguf.py").exists():
        ...
        if not update_llama_cpp(llama_cpp_dir):
            logger.warning("Update failed, continuing with existing version...")
        else:
            should_build = True # Rebuild after update
            
            # Install new requirements if any
            requirements = llama_cpp_dir / "requirements.txt"
            if requirements.exists():
                logger.info("Installing updated llama.cpp Python requirements...")
                subprocess.run(
                    [sys.executable, "-m", "pip", "install", "-r", str(requirements)],
                    check=True,
                    capture_output=True,
    ...
    logger.info(f"✅ llama.cpp binaries ready")
    return True
...
Quantization options (smallest to largest):
  ...
  f16     - No quantization (largest, best quality)
    ...
    parser.add_argument(
        "--update",
        action="store_true",
        help="Update llama.cpp (git pull) and rebuild before converting"
    )
    ...
    if not args.skip_build:
        if not ensure_llama_cpp_built(llama_cpp, cuda_enabled, args.update):
    ...
    if not convert_to_gguf(model_to_convert, f16_gguf, llama_cpp, outtype="f16"):
        ...
        if not args.update:
             logger.info("💡 Tip: Try running with --update to fetch the latest llama.cpp version which may fix converter errors.")
        sys.exit(1)
