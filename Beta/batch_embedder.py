...
class ImprovedBatchEmbedder:
    ...
    def _process_mapping_format(self, convo: dict, convo_idx: int, strict_mode: bool) -> Generator[Dict[str, Any], None, None]:
        ...
        for msg_id, msg_data in mapping.items():
            try:
                ...
                # Skip system prompts and tool results — they're not conversational training data
                if author in ('system', 'tool'):
                    continue
    ...
    def _extract_message_content(self, message: dict) -> str:
        ...
        if isinstance(content, str):
            ...
            if "parts" in content and isinstance(content["parts"], list):
                parts = content["parts"]
                if parts:
                    # Join ALL string parts so multi-part messages (text + code blocks) are fully captured
    ...
    def load_and_embed_all(self, 
        ...
        def estimate_convo_items(path):
            """Estimate item count without loading large files into memory twice.
            For files >50MB, samples the first 200 conversations to extrapolate.
            """
            count = 0
            ...
            if file_size > large_file_threshold:
                try:
                    # Read only a prefix large enough to sample a few hundred conversations
                    sample_bytes = min(file_size, 2 * 1024 * 1024)  # 2 MB sample
                    with open(path, 'r', encoding='utf-8', errors='replace') as f:
                        raw = f.read(sample_bytes)
                    # Count approximate messages by looking for common message indicators
                    # For ChatGPT mapping format: count "\"role\":" occurrences
                    # For Claude chat_messages format: count "\"sender\":" or "\"text\":"
                    role_hits = raw.count('"role":')
                    sender_hits = raw.count('"sender":')
                    msg_hits = max(role_hits, sender_hits)
                    if msg_hits > 0:
                        # Scale up proportionally from the sample
                        count = int(msg_hits * (file_size / sample_bytes))
                    else:
                        # Fallback: rough estimate of 400 bytes per message
                        count = file_size // 400
                except Exception as e:
