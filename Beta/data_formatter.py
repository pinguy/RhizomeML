...
def clean_text(text: str) -> str:
    """Fix encoding issues and strip invisible/control characters without mutating content."""
    if not text:
        return text
    try:
    ...
    # e.g. "don9t" → "don't", "it9s" → "it's"
    text = re.sub(r"(?i)\b([a-z]+)9(?=(?:t|s|m|re|ve|ll|d)\b)", r"\1'", text)
    # e.g. "in9" at word boundary → "ing" (common OCR drop of trailing g)
    text = re.sub(r"(?i)(?<=in)9(?=\b|[^a-z])", "g", text)
    # e.g. "word9s" → "word's" (possessives)
    text = re.sub(r"(?i)\b([a-z]{2,})9(?=s\b)", r"\1'", text)

    # --- Punctuation collapse ---
    # e.g. "!!!" or "..." followed by quote → single punctuation + quote
    text = re.sub(r'([!?.,]){2,}["\']', r'\1"', text)

    # --- OCR 9em artifact ---
    text = re.sub(r'\b9em\b', 'em', text)

    # --- Final OCR 9 cleanup (remove stray 9s not between digits) ---
...
def validate_text(text: str, cfg: Config) -> bool:
    """Reject clearly malformed or trivial texts; keep everything else."""
    if not text or not text.strip():
        return False
    stripped = text.strip()
    length = len(stripped)
    if length < cfg.min_text_length:
        return False
    if length > cfg.max_text_length:
        return False
    # Reject texts where a single character dominates (>80%) — likely noise/corrupted
    if length > 30:
        most_common_count = Counter(stripped.lower()).most_common(1)[0][1]
        if most_common_count / length > 0.80:
            return False
    return True
