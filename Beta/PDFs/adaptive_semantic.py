...
class Config:
    # IO
    pdf_dir: str = './PDFs'
    pdf_file: Optional[str] = None   # if set, process only this filename (basename)
    ...
    qa_max_pairs_per_source: int = 20000
...
class PDFProcessor:
    ...
    def extract_pdfs(self) -> List[Dict]:
        ...
        if self.cfg.pdf_file:
            target = os.path.basename(self.cfg.pdf_file)
            files = [f for f in files if f == target]
            if not files:
                logger.error(f"--pdf-file '{target}' not found in {self.cfg.pdf_dir}")
...
class QABuilder:
    ...
    def _diverse_prompts(self, chunk_text: str, metadata: Dict) -> List[str]:
        ...
        # Concept-level templates — no raw chunk text embedded in question
        templates = [
        ...
        if key_terms:
            term = random.choice(key_terms[:8])
            templates.extend([
    ...
    def _dedup_qas(self, qa_list: List[Dict], sim_threshold=0.98) -> List[Dict]:
        if not qa_list:
            return []
    ...
    def build(self, knowledge_records: List[Dict]) -> List[Dict]:
        ...
        for src, entries in tqdm(by_src.items(), desc='Q&A by source'):
            ...
            cap = max(1, self.cfg.qa_max_pairs_per_source)
            ...
            for g in grouped[:cap]:
                ...
                for q in qs:
                    qa.append({
                        'text': f"<|user|>{q}<|assistant|>{text}<|endoftext|>",
                        'user': q,
                        'assistant': text,
                        'quality_metrics': {
                            'quality_score': 1.0,
...
python3 adaptive_semantic.py \\
  ...
  --ocr-preprocessing
    ...
    p.add_argument('--pdf-file', default=None,
                   help='Process only this PDF filename (basename). E.g. "Cosmos.pdf"')
