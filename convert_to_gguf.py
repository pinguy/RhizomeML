...
class UCSEnhancedChatBot:
    ...
    def chat_response_streaming(self, user_input: str, history: List, enable_tts: bool, 
        ...
        # 1. Track completed sentences as text streams in
        # 2. Generate TTS for each completed sentence
        # 3. Yield audio chunks as they're ready
        
        final_response = ""
        last_tts_end = 0  # Track where we've already sent to TTS
        pending_audio_queue = []  # Queue of audio chunks ready to play
        ...
        for partial_response in self.generate_response_streaming(
            ...
            if enable_tts and self.streaming_tts_processor and stream_tts and config.tts_streaming:
                # Look for new complete sentences since last TTS
                new_text = partial_response[last_tts_end:]
                
                # Find sentence boundaries in new text
                matches = list(sentence_end_pattern.finditer(new_text))
                
                if matches:
                    # Get text up to and including the last complete sentence
                    last_match = matches[-1]
                    sentence_end_pos = last_match.end()
                    complete_text = new_text[:sentence_end_pos].strip()
                    
                    if complete_text and len(complete_text) > 10:  # Minimum length for TTS
                        try:
                            # Generate TTS for the complete sentence(s)
                            # Collect chunks and create a wav bytes object
                            audio_chunks = []
                            ...
                            if audio_chunks:
                                # Concatenate and convert to wav bytes for Gradio streaming
                                ...
                                logger.debug(f"🔊 Streaming TTS chunk: {len(complete_text)} chars -> {len(audio_chunk)} bytes")
                        except Exception as e:
                            logger.warning(f"Streaming TTS chunk error: {e}")
                        
                        # Update position
                        last_tts_end += sentence_end_pos
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("🔊 Voice Settings", open=False):
                    ...
                    stream_tts_checkbox = gr.Checkbox(
                        ...
                        info="Generate audio progressively as Kokoro processes each sentence"
                    )
...
- Enable "Show Reasoning" to see thought process (for reasoning models like DeepSeek)
        ...
        def handle_chat_streaming(user_input_text, history, enable_tts_val, voice_val, 
                                  ...
                                  enable_streaming_val, stream_tts_val):
        ...
        send_btn.click(
            ...
            inputs=[user_input, chatbot_interface, enable_tts, voice_selection, 
                   ...
                   enable_streaming_checkbox, stream_tts_checkbox],
